...
protected:
    ...
    size_type m_dim;
    size_type m_cur_krylov_dim;
    matrix<value_type, backend_type> m_Q;
    vector<value_type, backend_type> m_w;
    matrix<value_type> m_H;
    matrix<value_type> m_Hv;
    real_type m_threshold;

    //two quantities useful for setting the error estimate
    vector<real_type> m_hk1k;
    vector<real_type> m_Avabs;
    real_type m_beta;

public:
    arnoldi_iteration() : m_max_krylov_dim(0), m_max_dim(0), m_krylov_dim(0), m_dim(0), m_threshold(std::numeric_limits<real_type>::epsilon()), m_beta(0) {}
    arnoldi_iteration(size_type krylov_dim, size_type max_dim) 
        try : m_max_krylov_dim(0), m_max_dim(0), m_threshold(std::numeric_limits<real_type>::epsilon()), m_beta(0)
    {
        resize(krylov_dim, max_dim);
    }
    catch(const std::exception& ex){std::cerr << ex.what() << std::endl;    RAISE_EXCEPTION("Failed to construct arnoldi iteration object.");}
    ...
    void resize(size_type krylov_dim, size_type dim)
    {
        try
        {
            ...
            CALL_AND_HANDLE(m_H.resize(krylov_dim, krylov_dim), "Failed to resize the resultant upper hessenberg matrix storing the operator in the krylov subspace.");
            CALL_AND_HANDLE(m_Hv.resize(krylov_dim, krylov_dim+1), "Failed to resize the resultant upper hessenberg matrix storing the operator in the krylov subspace.");
            CALL_AND_HANDLE(m_hk1k.resize(krylov_dim+1), "Failed to resize the hk1k array.");
            CALL_AND_HANDLE(m_Avabs.resize(krylov_dim+1), "Failed to resize the Avabs array.");
            m_Avabs.fill_zeros();
        }        
        catch(const std::exception& ex)
        {
    ...
    void reallocate(size_type krylov_dim, size_type dim)
    {
        try
        {
            ...
            CALL_AND_HANDLE(m_H.reallocate(krylov_dim, krylov_dim), "Failed to reallocate the resultant upper hessenberg matrix storing the operator in the krylov subspace.");
            CALL_AND_HANDLE(m_Hv.reallocate(krylov_dim, krylov_dim+1), "Failed to reallocate the resultant upper hessenberg matrix storing the operator in the krylov subspace.");
            CALL_AND_HANDLE(m_hk1k.reallocate(krylov_dim+1), "Failed to reallocate the hk1k array.");
            CALL_AND_HANDLE(m_Avabs.reallocate(krylov_dim+1), "Failed to reallocate the Avabs array.");
        }        
        catch(const std::exception& ex)
        {
            std::cerr << ex.what() << std::endl;
    ...
    void deallocate()
    {
        try
        {
            ...
            CALL_AND_HANDLE(m_H.deallocate(), "Failed to deallocate the resultant upper hessenberg matrix storing the operator in the krylov subspace.");
            CALL_AND_HANDLE(m_Hv.deallocate(), "Failed to deallocate the resultant upper hessenberg matrix storing the operator in the krylov subspace.");
            CALL_AND_HANDLE(m_Avabs.deallocate(), "Failed to deallocate Avabs array.");
            CALL_AND_HANDLE(m_hk1k.deallocate(), "Failed to deallocate hk1k array.");
        }
        catch(const std::exception& ex)
        {
            std::cerr << ex.what() << std::endl;
    ...
    bool partial_krylov_step(const vec_type& vec, real_type scale, size_type istart, size_type iend, Args&& ... args) 
    {
        try
        {
            ...
            for(size_type i=istart; i <= iend && !iteration_completed_early; ++i)
            {
                if(i == 0)
                {
                    auto q0 = m_Q[0];  q0.set_buffer(vec);  real_type fnorm = sqrt(real(dot_product(conj(q0), q0)));  q0 = q0/fnorm;
                    m_beta = fnorm;
                    cur_krylov_dim=i;

                    auto rq0 = q0.reinterpret_shape(vec.shape());
                    //we precompute the next element and store it in m_w
                    CALL_AND_HANDLE(compute_action(rq0, wp, std::forward<Args>(args)...), "Failed to compute the partial krylov subspace.  Failed to evaluate action on vector.");
                    m_w *= scale;
                    real_type norm = sqrt(real(dot_product(conj(m_w), m_w)));
                    m_hk1k(cur_krylov_dim) = norm;
                    m_Avabs(cur_krylov_dim) = norm;
                }
                else
                {
                    //now we orthogonalise the previously obtained action vector
                    for(size_type j=0; j<i; ++j)
                    {
                        auto qj = m_Q[j];
                        CALL_AND_HANDLE(m_Hv(i-1, j) = dot_product(conj(m_w), qj), "Failed to compute the partial krylov subspace.  Failed to compute overlap necessary for modified Gram-Schmitd.");
                        CALL_AND_HANDLE(m_w -= m_Hv(i-1, j)*qj, "Failed to compute partial krylov subspace.  Failed to orthogonalise vector.");
                    }
                    real_type norm = sqrt(real(dot_product(conj(m_w), m_w)));
                    cur_krylov_dim = i; 
                    ...
                    //and we only store the result of this into the Q array if we aren't at the final element
                    if(norm < m_threshold)
                    {
                        iteration_completed_early = true;
                        m_Avabs(cur_krylov_dim) = 0.0;
                    }
                    else
                    {
                        auto qi = m_Q[i]; 
                        CALL_AND_HANDLE(qi = m_w / norm, "Failed to assign the current working array element");
                        m_Hv(i-1, i) = norm;
                        auto rqi = m_Q[i].reinterpret_shape(vec.shape());
                        CALL_AND_HANDLE(compute_action(rqi, wp, std::forward<Args>(args)...), "Failed to compute the partial krylov subspace.  Failed to evaluate action on vector.");
                        m_w *= scale;
                        m_Avabs(cur_krylov_dim) = sqrt(real(dot_product(conj(m_w), m_w)));
                    }       
                }
            }
            CALL_AND_HANDLE(finalise_krylov_rep(cur_krylov_dim), "Failed to compute partial krylov subspace.  Failed when finalising the iteration.");
    ...
    real_type Avabs() const{return m_Avabs(m_cur_krylov_dim);}

    const real_type& beta() const{return m_beta;}
    const real_type& hk1k() const{return m_hk1k(m_cur_krylov_dim);}
    real_type& threshold(){return m_threshold;}
    const real_type& threshold() const{return m_threshold;}
    const size_type& current_krylov_dim() const{return m_cur_krylov_dim;}
